...
enum msg_type : uint8_t { 
    ...
    // Component apply transaction (receiver-driven reboot orchestration)
    msg_component_apply_request,    // Batched component apply request (receiver → transmitter)
    msg_component_apply_ack,        // Apply result + persisted status (transmitter → receiver)
...
// Batched component apply request (receiver -> transmitter)
typedef struct __attribute__((packed)) {
