...
void ConnectionStateManager::init() {
    ...
    tracked_connection_state_ = ESPNow::transmitter_connected;
...
bool ConnectionStateManager::is_transmitter_connected() {
    ...
    bool result = ESPNow::transmitter_connected;
...
bool ConnectionStateManager::set_transmitter_connected(bool connected) {
    ...
    bool changed = (ESPNow::transmitter_connected != connected);
    if (changed) {
        ESPNow::transmitter_connected = connected;
...
bool ConnectionStateManager::poll_connection_change() {
    ...
    bool current = ESPNow::transmitter_connected;
...
bool ConnectionStateManager::has_data_received() {
    if (!state_mutex_) return ESPNow::data_received;
    if (xSemaphoreTake(state_mutex_, pdMS_TO_TICKS(100)) != pdTRUE) return ESPNow::data_received;
    bool value = ESPNow::data_received;
    xSemaphoreGive(state_mutex_);
    return value;
}
...
bool ConnectionStateManager::set_data_received(bool received) {
    if (!state_mutex_) return false;
    if (xSemaphoreTake(state_mutex_, pdMS_TO_TICKS(100)) != pdTRUE) return false;
    bool changed = (ESPNow::data_received != received);
    ESPNow::data_received = received;
    if (received) {
        last_data_received_ms_ = millis();
    }
    xSemaphoreGive(state_mutex_);
...
uint32_t ConnectionStateManager::get_connection_uptime_ms() {
    ...
    uint32_t uptime = 0;
    if (ESPNow::transmitter_connected && connection_start_time_ms_ != 0) {
...
void ConnectionStateManager::get_all_state(bool& out_connected, int& out_channel, uint8_t* out_mac) {
    ...
    out_connected = ESPNow::transmitter_connected;
