...
static esp_err_t root_handler(httpd_req_t *req) {
    ...
    <div class='settings-card'>
        ...
        <div class='settings-row'>
            ...
            <input type='checkbox' id='wifiApEnabled' %WIFIAPENABLED% />
        </div>
        <div class='settings-row' id='apNameRow'>
            <label>AP Name:</label>
            <input type='text' value='%APNAME%' disabled />
        </div>
        <div class='settings-row'>
            <label>Static IP:</label>
            <input type='checkbox' id='staticIpEnabled' %STATICIP% />
        </div>
    ...
    <div class='settings-card'>
        ...
        <div class='settings-row'>
            ...
            <input type='checkbox' id='mqttEnabled' %MQTTENABLED% />
        </div>
    ...
    String script = R"rawliteral(
        window.onload = function() {
            ...
            // Request IP data from transmitter (triggers ESP-NOW request)
            fetch('/api/request_transmitter_ip')
                .then(response => response.json())
                .then(data => {
                    console.log('IP data request sent:', data);
                    if (data.success) {
                        // Wait a moment for data to arrive, then fetch it
                        setTimeout(fetchTransmitterIP, 500);
                    }
                })
                .catch(err => {
                    console.error('Failed to request IP data:', err);
                });
