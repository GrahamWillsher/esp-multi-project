...
namespace {
    ...
    struct RuntimeStatus {
        ...
        uint8_t time_source = 0;  // 0=unsynced, 1=NTP, 2=manual, 3=GPS
    };

    // Metadata (persisted to NVS)
    struct Metadata {
        bool received = false;
        bool valid = false;
        char env[32] = {0};
        ...
        uint32_t version = 0;  // Packed as major*10000 + minor*100 + patch
    };
...
void update_runtime_status(bool eth_conn) {
    ...
    if (eth_changed) {
        LOG_INFO("[STATUS_CACHE] Runtime: ETH=%s",
...
bool is_transmitter_connected() {
    ...
    // This is moved from TransmitterManager to avoid circular dependency
