...
- File: [espnowreciever_2/src/espnow/espnow_callbacks.cpp](espnowreciever_2/src/espnow/espnow_callbacks.cpp)
- `on_data_recv()` validates length and pushes raw message into `ESPNow::queue`.

### 4) Receiver: message routing
- File: [espnowreciever_2/src/espnow/espnow_tasks.cpp](espnowreciever_2/src/espnow/espnow_tasks.cpp)
...
### 6) Display functions
- File: [espnowreciever_2/src/display/display_core.cpp](espnowreciever_2/src/display/display_core.cpp)
...
What to check:
- Mutex creation in [espnowreciever_2/src/main.cpp](espnowreciever_2/src/main.cpp).
...
- Add a temporary, minimal diagnostic step to prove draw path:
  - After `displayInitialScreen()` in [espnowreciever_2/src/main.cpp](espnowreciever_2/src/main.cpp), call `display_soc(55)` and `display_power(500)` under `RTOS::tft_mutex`.
  - If this renders, display layer is OK and the problem is in data flow.

## Notes
- The receiver default values in [espnowreciever_2/src/globals.cpp](espnowreciever_2/src/globals.cpp) are SOC=50, Power=0. If real data equals defaults, display functions previously skipped updates. This is now handled by the first-data forcing change.
