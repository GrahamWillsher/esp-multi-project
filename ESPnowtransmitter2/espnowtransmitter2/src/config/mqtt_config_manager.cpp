...
bool MqttConfigManager::saveConfig(bool enabled,
    ...
    // Update runtime snapshot used by ACK responses
    enabled_ = enabled;
    server_ = server;
    port_ = port;
...
void MqttConfigManager::applyConfig() {
    ...
    LOG_INFO("MQTT_CFG", "Configuration persists now and is applied on next reboot");
}
