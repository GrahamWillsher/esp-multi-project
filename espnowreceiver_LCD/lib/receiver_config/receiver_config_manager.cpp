...
bool ReceiverNetworkConfig::loadConfig() {
    ...
    battery_type_ = prefs.getUChar(NVS_KEY_BATTERY_TYPE, 29);
    inverter_type_ = prefs.getUChar(NVS_KEY_INVERTER_TYPE, 0);

    // Load Battery and Inverter interface selection (with defaults)
    battery_interface_ = prefs.getUChar(NVS_KEY_BATTERY_INTERFACE, 2);
    inverter_interface_ = prefs.getUChar(NVS_KEY_INVERTER_INTERFACE, 2);
...
bool ReceiverNetworkConfig::saveConfig(
    ...
    // Always save static IP configuration
    ...
    } else if (mqtt_password && strcmp(mqtt_password, "********") == 0) {
        prefs.getString(NVS_KEY_MQTT_PASSWORD, mqtt_password_, sizeof(mqtt_password_));
    }
