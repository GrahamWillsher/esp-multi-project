...
// Type aliases for Battery Emulator enums (resolve naming conflicts)
typedef BatteryEmulator_bms_status_enum bms_status_enum;
typedef BatteryEmulator_real_bms_status_enum real_bms_status_enum;
// led_mode_enum and balancing_status_enum are already defined in types.h correctly
...
class DataLayer {
 public:
  ...
  // Compatibility getters for existing code that expects datalayer.status.*
  inline DATALAYER_BATTERY_STATUS_TYPE& get_status() { return battery.status; }
  
  // Property to track if datalayer is initialized
  bool initialized = false;
  
  // Last CAN message timestamp for monitoring
  uint32_t last_can_message_timestamp = 0;
};
