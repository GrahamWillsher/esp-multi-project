...
static esp_err_t api_ota_upload_handler(httpd_req_t *req) {
    ...
    if (httpCode == 200) {
        http.end();
        LOG_INFO("OTA: Transmitter OTA started. Waiting for reboot sequence...");
        
        // Wait 15 seconds for transmitter OTA validation and reboot
        // This gives the transmitter time to:
        // 1. Finalize the firmware write (Update.end)
        // 2. Validate the firmware
        // 3. Reboot and reconnect
        vTaskDelay(pdMS_TO_TICKS(15000));
        
        LOG_INFO("OTA: Transmitter should be rebooting now");
