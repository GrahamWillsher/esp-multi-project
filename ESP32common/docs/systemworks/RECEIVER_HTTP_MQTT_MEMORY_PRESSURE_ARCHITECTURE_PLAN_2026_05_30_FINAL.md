...
### 5.A. API-first Web Delivery (thin shell pages)
...
### 5.B. Endpoint Admission Classes + Degraded Mode
...
### 5.C. Snapshot-Based Diagnostics (no heavy assembly in request path)
...
### 5.D. Strict Paging and Size Budgets
...
### 5.E. PSRAM as Pressure Relief (not core fix)
...
> **Context:** The recommended Phase 2A approach focuses on event-triggered logging for memory diagnostics. Before implementing, this appendix examines whether real-time polling or on-demand event logging better serves root-cause analysis of heap fragmentation. This comparison directly informs the decision to transition Memory Health pages from continuous polling to event-driven observability.
...
> **Bridge Note:** Section 8 provides the methodological foundation for implementing Phase 2A (Event Logging). It includes the fragmentation root-cause analysis workflow, multi-layer logging strategy, and instrumentation patterns needed to transform Memory Health diagnostics from polling-based to event-driven.
...
**Effort estimate:** ~8 hours (ring log implementation + endpoint + UI update).
...
**Effort estimate:** ~6 hours (audit + limit enforcement + validation).
...
**Effort estimate:** ~6 hours (classification + logic + integration).
...
**Effort estimate:** ~15 hours (PSRAM audit + migration + validation + soak testing).
...
**Effort estimate:** ~20 hours (72h soak + analysis + threshold calibration).

---

## 8) Root-Cause Analysis: Logging Strategies for Fragmentation Diagnosis

> **Purpose:** This section provides the methodological foundation for Phase 2A implementation. It details the instrumentation approach, multi-layer logging strategy, and step-by-step investigation workflow needed to diagnose fragmentation root causes using the event log approach.
...
- Immediate implementation cost: low (~8 hours).
...
### Phase 2A (Event Logging) ✅ Implementation Complete (Build-Validated)
1. ✅ `/api/memory_events` endpoint implemented in both receiver variants (`espnowreceiver_2`, `espnowreceiver_LCD`) with bounded event-ring response.
2. ✅ Memory Health page switched from auto-polling to on-load fetch + manual refresh (polling timer removed).
3. ✅ Legacy route and usage removed: `/api/memory_samples` de-registered and replaced by `/api/memory_events` in handlers, middleware classification, and Memory Health client flow.
4. ⏳ Runtime measurement pending: validate request-rate drop from ~15/2min to ~1/page-load on target device.
5. ⏳ Runtime measurement pending: confirm event stream captures threshold/recovery transitions under induced pressure.

### Phase 2B (Paging)
1. All pageable endpoints enforce limits (max 25–32 items per page).
2. `/api/pressure_gate_stats` returns ≤ 2KB response.
3. No unbounded array returns in any JSON response.

### Phase 2C (Admission Control)
1. When `largest_8bit < 8KB`, C2 endpoints return 503.
2. C0 (MQTT) endpoints remain available.
3. Event log records rejection count + last rejected endpoint.
...
3. **Feasibility:** Bounded ring log (4 KB) requires minimal code change (~8 hours).
...
- [Document Review Findings](./DOCUMENT_REVIEW_FINDINGS_2026_05_30.md) - Comprehensive audit and completeness analysis

---

## 13) Phase Completion Log

### 2026-05-30 – Phase 2A Implementation Update

**Completed in code (both variants):**
- Added bounded memory event ring + event copy API in MemorySampler.
- Added `/api/memory_events` telemetry endpoint and removed active use of `/api/memory_samples`.
- Updated Memory Health page data model from samples to events.
- Removed recurring polling behavior and retained manual refresh workflow.

**Legacy/redundant cleanup completed for this phase:**
- Deprecated route registration for `/api/memory_samples` removed from active API route table.
- Legacy Memory Health polling path and obsolete sample-state helper removed from active page script flow.
- Middleware URI classification updated to new `/api/memory_events` path.

**Validation evidence:**
- `espnowreceiver_LCD`: build success.
- `espnowreceiver_2`: build success (both target environments succeeded).

**Remaining to close full Phase 2A acceptance:**
- Device runtime verification for measured request-rate reduction and pressure-event capture quality.

---

**Document Version:** 2026-05-30 (Revised with logging strategy analysis + Phase 2A implementation update)  
**Status:** Architecture approved; Phase 2A implementation complete (build-validated), runtime verification pending.  
**Review Summary:** 94/100 overall - production ready with recommended cosmetic fixes applied.
