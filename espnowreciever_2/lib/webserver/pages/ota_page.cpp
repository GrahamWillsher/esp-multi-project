...
static esp_err_t ota_handler(httpd_req_t *req) {
    ...
    <div class='info-box' style='margin-bottom: 20px;'>
        ...
        <table style='width: 100%; border-collapse: collapse; margin-top: 15px;'>
            ...
            <tr style='border-bottom: 1px solid #444;'>
                ...
                <td style='padding: 10px; color: #4CAF50;'>)rawliteral";
    
    content += receiver_version;
    content += R"rawliteral(</td>
                <td style='padding: 10px; color: #888; font-size: 12px;'>)rawliteral";
    content += String(__DATE__) + " " + String(__TIME__);
    content += R"rawliteral(</td>
            </tr>
            <tr>
    ...
    String script = R"rawliteral(
        ...
        window.onload = function() {
            ...
            // Periodically refresh version info
            setInterval(function() {
                fetch('/api/version')
                    ...
                    .then(data => {
                        ...
                        if (data.transmitter_version !== 'Unknown') {
                            versionEl.innerText = data.transmitter_version;
                            
                            if (data.transmitter_build_date && data.transmitter_build_time) {
                                buildEl.innerText = data.transmitter_build_date + ' ' + data.transmitter_build_time;
                            }
                            
                            if (data.transmitter_compatible) {
                                statusEl.innerText = '✅ Compatible';
                                statusEl.style.color = '#4CAF50';
                            } else {
                                statusEl.innerText = '⚠️ Version mismatch';
