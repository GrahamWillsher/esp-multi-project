...
namespace {
static const httpd_uri_t kCoreApiHandlers[] = {
    ...
    {.uri = "/api/ota_upload", .method = HTTP_POST, .handler = api_ota_upload_handler, .user_ctx = NULL},
        ...
        {.uri = "/api/memory_samples", .method = HTTP_GET, .handler = api_memory_samples_handler, .user_ctx = NULL}
};
}

int register_all_api_handlers(httpd_handle_t server) {
    int count = 0;
    for (int index = 0; index < static_cast<int>(sizeof(kCoreApiHandlers) / sizeof(httpd_uri_t)); ++index) {
        if (httpd_register_uri_handler(server, &kCoreApiHandlers[index]) == ESP_OK) {
            count++;
        }
    }
...
int expected_all_api_handlers() {
    const int core = static_cast<int>(sizeof(kCoreApiHandlers) / sizeof(httpd_uri_t));
