...
void TftDisplay::animate_backlight(uint8_t target, uint32_t duration_ms) {
    ...
    for (uint32_t i = 0; i <= steps; i++) {
        ...
        // Use 16-bit fixed point to avoid rounding errors
        int32_t start = current_backlight_;
        int32_t delta = (int32_t)target - start;
        int32_t brightness_fp = (start << 16) + (delta << 16) * (int32_t)i / (int32_t)steps;
        uint8_t brightness = (uint8_t)(brightness_fp >> 16);
