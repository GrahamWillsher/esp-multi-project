...
static constexpr uint32_t kWebserverStartupTaskStack = 8192;
static constexpr UBaseType_t kWebserverStartupTaskPriority = 1;

static void webserver_startup_task(void* /*arg*/) {
    // Delay slightly so WiFi setup can settle before first init attempt.
    ...
    while (server == nullptr) {
        ++attempts;
        LOG_INFO("MAIN", "Webserver init attempt %lu", static_cast<unsigned long>(attempts));
        init_webserver();
        if (server != nullptr) {
            break;
        }
    ...
    LOG_INFO("MAIN", "Webserver startup task complete (attempts=%lu)",
...
// --- Phase 4: Services -------------------------------------------------------
// Receiver/transmitter caches, webserver startup, WiFi power tuning.
// Depends on: WiFi (Phase 3).
static void bootstrap_services() {
    ReceiverConfigManager::init();
    TransmitterManager::init();

    const BaseType_t ws_task_rc = xTaskCreate(
...
void setup() {
    ...
    LOG_INFO("MAIN", "Setup complete! All 5 bootstrap phases done.");
}
