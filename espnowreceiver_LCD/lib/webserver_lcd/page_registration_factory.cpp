...
int register_all_pages(httpd_handle_t server) {
    ...
    // Register AP/setup-compatible lightweight network page at /config.
    // Intentionally do not register NetworkPage::register_root() because
    // '/' is owned by dashboard_handler.
    if (NetworkPage::register_handler(server) == ESP_OK) {
        registered_count++;
        LOG_DEBUG("PAGE_FACTORY", "✓ /config (%d/%d + extras)", registered_count, PAGE_HANDLER_COUNT);
    } else {
        LOG_WARN("PAGE_FACTORY", "✗ /config - Error registering network setup page");
    }
    
    const int expected_total = get_expected_page_handler_count();
    LOG_INFO("PAGE_FACTORY", "Page registration complete: %d/%d successful", 
             registered_count, expected_total);
    
    return registered_count;
}

int get_expected_page_handler_count() {
    return PAGE_HANDLER_COUNT + 1; // + /config
}
