...
// Transmitter OTA status proxy API
static esp_err_t api_transmitter_ota_status_handler(httpd_req_t *req) {
    if (!TransmitterManager::isIPKnown()) {
        httpd_resp_set_type(req, "application/json");
...
// OTA firmware upload API (streams directly to transmitter)
static esp_err_t api_ota_upload_handler(httpd_req_t *req) {
    ...
    // Send HTTP request headers to transmitter
    ...
    // Stream body from browser request to transmitter socket
    char buf[1024];
    ...
    // Wait for transmitter response headers
...
static esp_err_t api_version_handler(httpd_req_t *req) {
    ...
    // Receiver firmware info: authoritative source is embedded metadata in .bin
...
static esp_err_t api_get_led_runtime_status_handler(httpd_req_t *req) {
    char json[512];
...
static esp_err_t api_resync_led_state_handler(httpd_req_t *req) {
    char json[256];
...
int register_all_api_handlers(httpd_handle_t server) {
    ...
    // Register type/interface selection APIs from modular handler file
