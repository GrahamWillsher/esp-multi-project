...
static const ApiMiddleware::RouteContext kCoreApiHandlers[] = {
    ...
    {.uri = "/api/cell_data/subscribe", .method = HTTP_POST, .handler = api_cell_data_subscribe_handler, .policy = ApiMiddleware::RoutePolicy::MutatingNoBody},
    {.uri = "/api/cell_data/unsubscribe", .method = HTTP_POST, .handler = api_cell_data_unsubscribe_handler, .policy = ApiMiddleware::RoutePolicy::MutatingNoBody},
    {.uri = "/api/cell_data/keepalive", .method = HTTP_POST, .handler = api_cell_data_keepalive_handler, .policy = ApiMiddleware::RoutePolicy::MutatingNoBody},
...
int register_all_api_handlers(httpd_handle_t server) {
    ...
    if (ApiNetwork::register_handlers(server) == ESP_OK) {
        count += 2; // GET + POST /api/v1/network
    }
...
int expected_all_api_handlers() {
    ...
    const int api_network_v1 = 2;
    const int wildcard_notfound = 1;
    return core + type_selection + api_network_v1 + wildcard_notfound;
}
